...
def get_image(filename, subfolder, folder_type):
    try:
        ...
        with urllib.request.urlopen(f"http://{COMFYUI_SERVER_ADDRESS}/view?{url_values}", timeout=900) as response:
...
def get_history(prompt_id):
    try:
        with urllib.request.urlopen(f"http://{COMFYUI_SERVER_ADDRESS}/history/{prompt_id}", timeout=900) as response:
...
async def add_to_queue(interaction: discord.Interaction, prompt: str, generation_type: str):
    ...
    async with server_queue['lock']:
        ...
        if position == 1 and not server_queue['currently_processing']:
            ...
            # Store the original message for future edits
...
class MultiServerDuckBot(commands.Bot):
    ...
    async def on_guild_join(self, guild):
        ...
        if guild.system_channel:
            embed = discord.Embed(
                title="🦆 DuckBot v2.2.3 Multi-Server Edition",
...
async def comfyui_health_command(interaction: discord.Interaction):
    ...
    try:
        response = requests.get(f"http://{COMFYUI_SERVER_ADDRESS}/", timeout=900)
    ...
    if "✅" in http_status:
        try:
            models_response = requests.get(f"http://{COMFYUI_SERVER_ADDRESS}/object_info", timeout=900)
...
async def ping_command(interaction: discord.Interaction):
    await interaction.response.send_message("Pong! 🦆 DuckBot v2.2.3 Multi-Server is running!", ephemeral=False)
...
async def generate_command(interaction: discord.Interaction, prompt: str):
    try:
        ...
        print(f"⚠️  Generate command interaction expired for user {interaction.user.name}")
        # Try to send a followup if possible
        try:
            await interaction.followup.send("❌ Request timed out - please try again", ephemeral=True)
        except:
            pass  # Can't do anything if followup also fails
...
async def animate_command(interaction: discord.Interaction, prompt: str):
    try:
        ...
        print(f"⚠️  Animate command interaction expired for user {interaction.user.name}")
        # Try to send a followup if possible
        try:
            await interaction.followup.send("❌ Request timed out - please try again", ephemeral=True)
        except:
...
if __name__ == "__main__":
    if not DISCORD_TOKEN:
        ...
        print("Starting DuckBot v2.2.3 Multi-Server Edition (Enhanced Adventures)...")
