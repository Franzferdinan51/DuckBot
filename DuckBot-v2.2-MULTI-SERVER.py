...
async def process_ask_generation(queue_item: QueueItem):
    ...
    try:
        ...
        if not health_check:
            ...
            await queue_item.status_message.delete()
            return
...
Keep responses under 1500 characters when possible to fit Discord\'s message limits well."""
        ...
        if data and "choices" in data and len(data["choices"]) > 0:
            ...
            if ai_response and ai_response.strip():
                await queue_item.status_message.delete() # Clean up
        ...
        try:
            ...
            fallback_msg = await get_fallback_response(prompt, user_name)
            await interaction.followup.send(fallback_msg)
        except: # Ignore if we can't send a message
            pass
...
async def enhanced_ask_command_multi_server(interaction: discord.Interaction, prompt: str):
    ...
    ask_queue = get_ask_queue(interaction.guild.id)
    queue_item = QueueItem(interaction, prompt, "ask", interaction.guild.id)
    ...
    if not ask_queue['currently_processing']:
        asyncio.create_task(process_ask_queue(interaction.guild.id))
